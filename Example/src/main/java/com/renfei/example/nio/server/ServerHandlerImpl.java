...
//            socketChannel.register(selectionKey.selector(), SelectionKey.OP_CONNECT, ByteBuffer.allocate(bufferSize));
        } else {
            //将channel改为读取状态
            buffer.flip();
            //按照编码读取数据
            ...
            // 注册一个写
            socketChannel.register(selectionKey.selector(), SelectionKey.OP_WRITE, ByteBuffer.allocate(bufferSize));
...
//            socketChannel.write(buffer);
    ...
    @Override
    public String write(SelectionKey selectionKey, String receivedStr) throws IOException {
        SocketChannel socketChannel = (SocketChannel) selectionKey.channel();
        ByteBuffer buffer = (ByteBuffer) selectionKey.attachment();
